...
namespace PzG {
   ...
   *  \li \p  SR_Ciagly - rysowana jest linia łaman łącząca kolejne punkty.
   *                    Współrzędne punktów zadawane są w pliku tekstowym.
   *  \li \p  SR_Punktowy - 
   *  \li \p  SR_PunktowyNiejednorodny - pozwala każdemu punktowi zadawać
   *                    indywidualnie jego rozmiar.
   */
  enum SposobRysowania { SR_Ciagly, SR_Punktowy, SR_PunktowyNiejednorodny }; 
  ...
  class InfoPlikuDoRysowania {
    public:
      ...
      */
     SposobRysowania WezSposobRys() const { return _SposobRys; }
     /*!
...
class LaczeDoGNUPlota {
 ...
 public:
  ...
  void WyswietlajKomunikatyBledow( bool Tryb = true );
    ...
    * \param  SposobRysowania - określa rodzaje rysowania danych
    *               przekazanych w pliku. Wybrany rodzaj rysowania
    *               narzuca też pewne dodatkowe wymogi odnośnie
    *               formatowania pliku.
    ...
    */
    bool DodajNazwePliku( const char       * NazwaPliku, 
                          SposobRysowania    SposobRys = SR_Ciagly, 
                          int                Szerokosc = 1
                        );
